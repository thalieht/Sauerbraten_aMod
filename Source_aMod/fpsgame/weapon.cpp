...
namespace game
{
    ...
    VARP(randomtrailcolor, 0, 1, 1); // (input_name_variable, min value, current_value (default), max_value) // toggle random trail color options
    VARP(trailcolorperteam, 0, 1, 1);                            // toggle if shot colors are determined by team
    VARP(randomcolorpertrail, 0, 1, 1);                          // toggle: 0==Shot color is random but same color for all trails per shot  1==ALL shot colors are random and independent from each other.
    VARP(blueteamsmoke, 0, 1, 1);                                // toggle blue team's smoke trail
    VARP(blueteamlightning, 0, 0, 1);                            // toggle blue team's lightning trail
    VARP(blueteamlaser, 0, 0, 1);                                // toggle blue team's laser trail
    VARP(redteamsmoke, 0, 0, 1);                                 // toggle red team's smoke trail
    VARP(redteamlightning, 0, 1, 1);                             // toggle red team's lightning trail
    VARP(redteamlaser, 0, 0, 1);                                 // toggle red team's laser trail
    ...
    VARP(bluesmokeparticle, 0, 0, 5);                            // choose particle for blue team's smoke trail
    VARP(redsmokeparticle, 0, 0, 5);                             // choose particle for red team's smoke trail
    VARP(grav, -50, 20, 50);                                     // gravity for smoke trail. the closer to zero, the faster it travels
    VARP(lightningduration, 1, 900, 10000); // in milliseconds   // duration of lightning trail
    VARP(smokeduration,1, 600, 10000);      // in milliseconds   // duration of smoke trail
    VARP(laserduration,1, 1400, 10000);     // in milliseconds   // duration of laser trail
    HVARP(blueteamlightningcolor, 0x000000, 0xFF01FF, 0xFFFFFF); // input variable from gui
    HVARP(blueteamsmokecolor, 0x000000, 0xFF01FF, 0xFFFFFF);     // input variable from gui
    HVARP(blueteamlasercolor, 0x000000, 0xFF01FF, 0xFFFFFF);     // input variable from gui
    HVARP(redteamlightningcolor, 0x000000, 0xFF01FF, 0xFFFFFF);  // input variable from gui
    HVARP(redteamsmokecolor, 0x000000, 0xFF01FF, 0xFFFFFF);      // input variable from gui
    HVARP(redteamlasercolor, 0x000000, 0xFF01FF, 0xFFFFFF);      // input variable from gui
    HVARP(globallightningcolor, 0x000000, 0xFF01FF, 0xFFFFFF);   // input variable from gui
    HVARP(globalsmokecolor, 0x000000, 0xFF01FF, 0xFFFFFF);       // input variable from gui
    HVARP(globallasercolor, 0x000000, 0xFF01FF, 0xFFFFFF);       // input variable from gui
    ...
    void shoteffects(int gun, const vec &from, const vec &to, fpsent *d, bool local, int id, int prevaction)     // create visual effect from a shot
    {
        ...
        switch(gun)
        {
            ...
            case GUN_RIFLE:
                int BlueTeamSmokeParticleChosen = 0;
                switch(bluesmokeparticle)
                {
                    case 0: BlueTeamSmokeParticleChosen = PART_SMOKE; break; // if RI_PARTb == 0 put the value of PART_SMOKE in RI_PARb
                    case 1: BlueTeamSmokeParticleChosen = PART_STEAM; break;
                    case 2: BlueTeamSmokeParticleChosen = PART_WATER; break; // else if RI_PARTb == 2 put the value of PART_WATER in RI_PARb
                    case 3: BlueTeamSmokeParticleChosen = PART_SNOW;  break;
                    case 4: BlueTeamSmokeParticleChosen = PART_FLAME; break;
                    case 5: BlueTeamSmokeParticleChosen = PART_SPARK; break;
                }
                int RedTeamSmokeParticleChosen = 0;
                switch(redsmokeparticle)
                {
                    case 0: RedTeamSmokeParticleChosen = PART_SMOKE; break;
                    case 1: RedTeamSmokeParticleChosen = PART_STEAM; break;
                    case 2: RedTeamSmokeParticleChosen = PART_WATER; break;
                    case 3: RedTeamSmokeParticleChosen = PART_SNOW;  break;
                    case 4: RedTeamSmokeParticleChosen = PART_FLAME; break;
                    case 5: RedTeamSmokeParticleChosen = PART_SPARK; break;
                }
            int LightningColorChosen, SmokeColorChosen, LaserColorChosen;
            if(trailcolorperteam)
            {
            // Comment: if player X is in same team as you (blue) we use the color from inputLig/Smok/LasBLUE variable, else from inputLig/Smok/LasRED
                ...
                isteam(d->team, player1->team) ? LightningColorChosen = blueteamlightningcolor : LightningColorChosen = redteamlightningcolor;
                isteam(d->team, player1->team) ? SmokeColorChosen     = blueteamsmokecolor     : SmokeColorChosen     = redteamsmokecolor;
                isteam(d->team, player1->team) ? LaserColorChosen     = blueteamlasercolor     : LaserColorChosen     = redteamlasercolor;
            }
            else if(!trailcolorperteam)
            {
                LightningColorChosen = globallightningcolor;
                SmokeColorChosen     = globalsmokecolor;
                LaserColorChosen     = globallasercolor;
            }
            if(randomtrailcolor)
            {
                switch(randomcolorpertrail)
                {
                    case 0: // same color for all trails per shot
                    {
                        long colorbuffer     = rnd(16777216);
                        LightningColorChosen = colorbuffer;
                        SmokeColorChosen     = colorbuffer;
                        LaserColorChosen     = colorbuffer;
                        break;
                    }
                    case 1: // different color per trail per shot
                    {
                        LightningColorChosen  = rnd(16777216);
                        SmokeColorChosen      = rnd(16777216);
                        LaserColorChosen      = rnd(16777216);
                        break;
                    }
                }
            }
            particle_splash(PART_SPARK, 200, 250, to, 0xB49B4B, 0.24f, 150); // already existed
            if(blueteamlightning && redteamlightning) // must be first, otherwise if any of the others is 1 it wont execute the else if's
                particle_flare(hudgunorigin(gun, from, to, d), to, lightningduration, PART_LIGHTNING, LightningColorChosen, lightningtrailwidth);
            else if(blueteamlightning)
            {
                isteam(d->team, player1->team) ? particle_flare(hudgunorigin(gun, from, to, d), to, lightningduration, PART_LIGHTNING, LightningColorChosen, lightningtrailwidth)
                : particle_flare(hudgunorigin(gun, from, to, d), to, lightningduration, NULL, 3, lasertrailwidth); // disabler (replaced color with 3 and its disabled)
            }
            else if(redteamlightning)
            {
                !isteam(d->team, player1->team) ? particle_flare(hudgunorigin(gun, from, to, d), to, lightningduration, PART_LIGHTNING, LightningColorChosen, lightningtrailwidth)
                : particle_flare(hudgunorigin(gun, from, to, d), to, lightningduration, NULL, 3, lasertrailwidth); // disabler (replaced color with 3 and its disabled)
            }
            if(blueteamsmoke && redteamsmoke) // must be first, otherwise if any of the others is 1 it wont execute the else if's
                particle_trail(isteam(d->team, player1->team) ? BlueTeamSmokeParticleChosen : RedTeamSmokeParticleChosen , smokeduration, hudgunorigin(gun, from, to, d), to, SmokeColorChosen, smoketrailwidth, grav);
            else if(blueteamsmoke)
            {
                isteam(d->team, player1->team) ? particle_trail(BlueTeamSmokeParticleChosen, smokeduration, hudgunorigin(gun, from, to, d), to, SmokeColorChosen, smoketrailwidth, grav)
                : particle_flare(hudgunorigin(gun, from, to, d), to, smokeduration, NULL, 3, lasertrailwidth); // disabler (replaced color with 3 and its disabled)
            }
            else if(redteamsmoke)
            {
                !isteam(d->team, player1->team) ? particle_trail(RedTeamSmokeParticleChosen, smokeduration, hudgunorigin(gun, from, to, d), to, SmokeColorChosen, smoketrailwidth, grav)
                : particle_flare(hudgunorigin(gun, from, to, d), to, smokeduration, NULL, 3, lasertrailwidth); // disabler (replaced color with 3 and its disabled)
            }
            if(blueteamlaser && redteamlaser) // must be first, otherwise if any of the others is 1 it wont execute the else if's
                particle_flare(hudgunorigin(gun, from, to, d), to, laserduration, PART_STREAK, LaserColorChosen, lasertrailwidth);
            else if(blueteamlaser)
            {
                isteam(d->team, player1->team)  ?  particle_flare(hudgunorigin(gun, from, to, d), to, laserduration, PART_STREAK, LaserColorChosen, lasertrailwidth)
                : particle_flare(hudgunorigin(gun, from, to, d), to, laserduration, NULL, 3, lasertrailwidth);
            }
            else if(redteamlaser)
            {
                !isteam(d->team, player1->team) ? particle_flare(hudgunorigin(gun, from, to, d), to, laserduration, PART_STREAK, LaserColorChosen, lasertrailwidth)
                : particle_flare(hudgunorigin(gun, from, to, d), to, laserduration, NULL, 3, lasertrailwidth); // disabler (replaced color with 3 and its disabled)
            }
         //       particle_splash(PART_SPARK, 200, 250, to, 0xB49B4B, 0.24f);
